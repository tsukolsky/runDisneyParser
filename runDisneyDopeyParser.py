...
class Partcipant():
    def __init__(self, line : str):
        ...
        firstPos = self.__GetFirstTimePosition(line)
        lastPos = self.__GetLastTimePosition(line)
    ...
    def __str__(self):
        return f"{self.line}\nPartcipant: {self.name} {self.age} {self.sex} {self.origin}\n{self.stringTime}\n{self._times}"
    ...
    def __ConvertToSeconds(self, timestring : str) -> int:
    ...
    def __GetFirstTimePosition(self, middleString) -> int:
    ...
    def __GetLastTimePosition(self, middleString) -> int:
    ...
    def ParseTimes(self, times : str):
        ...
        # for that
        if len(times) > 0:
            numSec = self.__ConvertToSeconds(times[-1])
            individualResult[FULL] = numSec
        if len(times) > 1:
            numSec = self.__ConvertToSeconds(times[-2])
            individualResult[HALF] = numSec
        if len(times) > 2:
            numSec = self.__ConvertToSeconds(times[-3])
            individualResult[TENK] = numSec
        if len(times) > 3:
            numSec = self.__ConvertToSeconds(times[-4])
...
def ParsePDF(filename : str, outputfile : str = "tmp-markup.txt"):
...
def SetupArgparse():
    ...
    parser.add_argument('-t', '--times', type=int, help='Minimum number of times to look for.', default=3)
    parser.add_argument('-o', '--output', type=str, help='Output file for excel formatted data', default="output.csv")
    parser.add_argument('-d', '--delimiter', type=str, help='delimiter of data for csv file. default is semicolon \';\'', default=';')
...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        count = len(matches)
        if count >= args.times:
            part = Partcipant(line)
            dataList = [part.name, part.age, part.sex, part.origin, part.result[FIVEK], part.result[TENK], part.result[HALF], part.result[FULL], part.result[TOTAL_TIME]]
